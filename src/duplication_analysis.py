...
def main():
    ...
    labels = ['Naive Bayes only bug information', 'Naive Bayes with reputation analysis',
              ...
              'Logistic Regression only bug information', 'Logistic Regression with reputation analysis']
    # labels = ['Extreme Randomized Tree only bug information', 'Extreme Randomized Tree with reputation analysis',
    #           'Adaboost Class. only bug information', 'Adaboost Class. with reputation analysis',
    #           'Logistic Regression only bug information', 'Logistic Regression with reputation analysis']
    #CLF = [RandomForestClassifier(verbose=1), svm.SVC(verbose=True)]
    #CLF = [ExtraTreesClassifier(verbose=1),AdaBoostClassifier(),LogisticRegression()]
    ...
    for clf in CLF:
        for x_tr, x_tst in itertools.izip(X, X_t):
            ...
            results['predicted_'+labels[i].replace(" ","_")]=pd.Series(ms.cross_val_predict(clf, x_tst, y_test_real, cv=5))
